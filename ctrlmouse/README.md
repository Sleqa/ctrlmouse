...
| R1 / R2 | Forward / back, as the mouse side buttons |
...
| `bind_lclick` / `bind_rclick` / `bind_keyboard` / `bind_playpause` / `bind_fullscreen` / `bind_launcher` / `bind_toggle` / `bind_forward` / `bind_back` | Controller button index per action; rebindable from the settings window | `1, 2, 3, 0, 0, 9, 13, 5, 7` |
